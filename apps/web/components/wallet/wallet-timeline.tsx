      ...
      {entries.slice(0, 10).map((entry, idx) => (
        <div key={entry.id} className="relative flex gap-4 pb-4">
          {idx < Math.min(entries.length, 10) - 1 && (
            <div className="absolute left-[11px] top-6 h-full w-px bg-surface" />
          )}
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-text-primary">{entry.type.replace(/_/g, ' ')}</span>
            ...
            <p className="mt-0.5 truncate text-xs text-text-tertiary">{entry.reason}</p>
            <div className="mt-0.5 flex items-center gap-2">
              <span className="text-[10px] text-text-tertiary">{new Date(entry.createdAt).toLocaleDateString()}</span>
