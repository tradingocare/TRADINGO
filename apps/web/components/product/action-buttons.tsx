...
const ACTIVE_BG = 'color-mix(in srgb, var(--accent) 20%, transparent)';
    ...
    <div className={cn('flex flex-wrap items-center gap-3', className)}>
      <div className="flex items-center rounded-xl overflow-hidden"
        ...
        <button onClick={() => onQuantityChange(Math.max(1, quantity - 1))}
          className="flex h-10 w-10 items-center justify-center text-text-tertiary hover:text-primary hover:bg-surface-tertiary transition-all" aria-label="Decrease quantity">
          <Minus size={14} />
        </button>
        <input type="number" value={quantity}
          onChange={(e) => { const v = parseInt(e.target.value, 10); if (!isNaN(v) && v >= 1) onQuantityChange(v); }}
          className="h-10 w-14 text-center text-sm font-bold text-text-primary bg-transparent focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
          style={{ borderLeft: GLASS.border, borderRight: GLASS.border }} min={1} />
        <button onClick={() => onQuantityChange(quantity + 1)}
          className="flex h-10 w-10 items-center justify-center text-text-tertiary hover:text-primary hover:bg-surface-tertiary transition-all" aria-label="Increase quantity">
          <Plus size={14} />
        </button>
      </div>
      ...
      <button onClick={onWishlist}
        ...
        style={{
          background: isWishlisted ? ACTIVE_BG : GLASS.bg,
          border: isWishlisted ? ACTIVE_BORDER : GLASS.border,
      ...
      <button onClick={onCompare}
        className="flex items-center justify-center w-10 h-10 rounded-xl transition-all"
        style={{
          background: isCompared ? ACTIVE_BG : GLASS.bg,
          border: isCompared ? ACTIVE_BORDER : GLASS.border,
      ...
      <button onClick={onChat}
        className="flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl text-[11px] font-bold transition-all"
        style={{ background: GLASS.bg, border: GLASS.border, color: 'var(--text-secondary)' }}>
      ...
      {directPurchaseEnabled && (
        <button onClick={onBuy}
          ...
          style={{ background: 'linear-gradient(135deg, var(--accent), var(--accent))', color: 'var(--text-on-accent)', boxShadow: '0 4px 16px color-mix(in srgb, var(--accent) 30%, transparent)' }}>
      ...
      {attachments && attachments.length > 0 && (
        <button onClick={onDownloadBrochure}
          ...
          style={{ background: GLASS.bg, border: GLASS.border, color: 'var(--text-tertiary)' }}>
