...
const CompanyCard = memo(function CompanyCard({
  company, index = 0, preview = false,
}: { company: CompanyCardData; index?: number; preview?: boolean }) {
  ...
  const handlePreviewClick = () => {
    if (!preview) return
    toast({
      title: 'Sample company preview',
      description: 'This is a demo company profile. Real company pages appear when sellers register.',
    })
  }
  ...
  const cardInner = (
    <motion.div
      ...
      className="relative rounded-2xl overflow-hidden cursor-pointer group h-full compact-stack-card ambient-backlight border border-border">
  ...
  if (preview) {
    return (
      <div className="stacked-card-wrapper h-full" onClick={handlePreviewClick} role="button" tabIndex={0}>
        {cardInner}
      </div>
    )
  }
