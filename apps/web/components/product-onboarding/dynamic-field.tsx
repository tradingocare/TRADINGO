...
export function DynamicField({ field, value, error, onChange }: DynamicFieldProps) {
 ...
 className="rounded-md border border-border bg-surface px-2.5 py-1 text-xs text-text-secondary transition-colors hover:border-primary-400 hover:text-primary-600 dark:hover:border-primary-500"
 >
 + {opt}
 </button>
 ))}
 ...
 className={cn(
 'text-sm font-medium',
 error ? 'text-red-600 dark:text-red-400' : 'text-text-primary',
 )}
 >
 {field.label}
