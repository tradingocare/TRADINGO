...
export default function SellerPoPage() {
  ...
  const { data: pos, isLoading, error } = useMyPurchaseOrders(filter || undefined);
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? <TableSkeleton /> : error ? (
        <EmptyState icon={AlertCircle} variant="error" title="Failed to load purchase orders" description="Please try again later." />
      ) : list.length === 0 ? (
        <EmptyState icon={FileText} title="No purchase orders yet" description="When buyers generate purchase orders, they will appear here." action={<Button variant="accent" className="mt-4" onClick={() => router.push('/seller/quote')}>View My Quotes</Button>} />
      ) : (
