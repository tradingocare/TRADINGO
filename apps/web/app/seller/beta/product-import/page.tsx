...
import { getProductImportGuide } from '@/lib/api/beta';
import { CheckCircle, Circle, Upload, Package, ArrowRight } from 'lucide-react';
...
export default function ProductImportPage() {
  ...
  const fetchGuide = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const result = await getProductImportGuide();
      setData(result);
    } catch {
      setError('Failed to load product import guide');
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      <DashboardPageHeader
        ...
        actions={
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => router.push('/seller/products/bulk-upload')}>
          ...
          {allDone && (
            <div className="mt-6 flex flex-col items-center justify-center rounded-lg border border-accent-500/30 bg-accent-50/30 p-6 text-center dark:border-accent-500/20 dark:bg-accent-900/10">
              ...
              <p className="font-medium text-accent-700 dark:text-accent-400">
                All import steps completed!
              </p>
              <p className="mt-1 text-sm text-text-secondary dark:text-dark-text-secondary">
                Your products are ready for the marketplace.
              </p>
            </div>
          )}
        </CardContent>
