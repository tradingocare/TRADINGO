...
import { ShoppingCart, User, Calendar, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <EmptyState icon={AlertCircle} variant="error" title="Failed to load orders" description="Please try again later." />
      ) : !data?.data?.length ? (
        <EmptyState icon={ShoppingCart} title="No orders yet" description="Orders from buyers will appear here." />
      ) : (
          ...
          {data.data.map((order: Order) => (
            <div key={order.id} className="grid grid-cols-1 gap-3 border-b border-border px-6 py-4 last:border-0 sm:grid-cols-12 sm:items-center dark:border-dark-border">
              ...
              <div className="sm:col-span-1 sm:text-right">
                <span className="text-xs font-medium text-primary-600 hover:underline cursor-pointer dark:text-primary-400" onClick={() => router.push(`/seller/order/${order.id}`)}>View</span>
              </div>
            </div>
          ))}
        </div>
