      ...
      <div className="space-y-5">
        <div>
          ...
          <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
            className="w-full px-4 py-3 rounded-xl text-text-primary text-sm bg-surface border border-border focus:outline-none focus:border-[#f59e0b] transition-colors"
            placeholder="Your business name" />
        </div>

        <div>
          <label className="text-text-secondary text-xs font-semibold mb-1.5 block">Trade / Brand Name</label>
          <input value={form.tagline} onChange={e => setForm(p => ({ ...p, tagline: e.target.value }))}
            className="w-full px-4 py-3 rounded-xl text-text-primary text-sm bg-surface border border-border focus:outline-none focus:border-[#f59e0b] transition-colors"
            placeholder="Shorter marketing name" maxLength={100} />
          <span className="text-text-tertiary text-xs mt-1 block">{form.tagline.length}/100</span>
        </div>

        <div>
          <label className="text-text-secondary text-xs font-semibold mb-1.5 block">Business Description *</label>
          <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
            className="w-full px-4 py-3 rounded-xl text-text-primary text-sm bg-surface border border-border focus:outline-none focus:border-[#f59e0b] transition-colors min-h-[100px]"
            placeholder="Describe your business (50-500 chars)" minLength={50} maxLength={500} />
          <span className="text-text-tertiary text-xs mt-1 block">{form.description.length}/500</span>
        </div>

        <div>
          <label className="text-text-secondary text-xs font-semibold mb-1.5 block">Seller Type *</label>
          <div className="flex flex-wrap gap-2">
            {SELLER_TYPES.map(t => (
              <button key={t} onClick={() => setForm(p => ({ ...p, sellerType: t }))}
                ...
                style={{
                  background: form.sellerType === t ? 'rgba(245, 158, 11, 0.15)' : '',
                  border: form.sellerType === t ? '1px solid rgba(245, 158, 11, 0.35)' : '1px solid var(--border-color)',
                  color: form.sellerType === t ? '#f59e0b' : 'rgba(255,255,255,0.6)',
                }}>{t}</button>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select value={form.yearEstablished} onChange={e => setForm(p => ({ ...p, yearEstablished: e.target.value }))}
              className="w-full px-4 py-3 rounded-xl text-text-primary text-sm bg-surface border border-border focus:outline-none focus:border-[#f59e0b] transition-colors">
              <option value="" className="bg-bg-base">Select year</option>
              {YEARS.map(y => <option key={y} value={y} className="bg-bg-base">{y}</option>)}
            </select>
          </div>
          <div>
            <label className="text-text-primary/70 text-xs font-semibold mb-1.5 block">Total Employees *</label>
            <select value={form.employeeCount} onChange={e => setForm(p => ({ ...p, employeeCount: e.target.value }))}
              className="w-full px-4 py-3 rounded-xl text-text-primary text-sm bg-surface border border-border focus:outline-none focus:border-[#f59e0b] transition-colors">
              <option value="" className="bg-bg-base">Select range</option>
              {EMP_COUNTS.map(e => <option key={e} value={e} className="bg-bg-base">{e}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-text-secondary text-xs font-semibold mb-1.5 block">Supply Capacity</label>
            <input value={form.supplyCapacity} onChange={e => setForm(p => ({ ...p, supplyCapacity: e.target.value }))}
              className="w-full px-4 py-3 rounded-xl text-text-primary text-sm bg-surface border border-border focus:outline-none focus:border-[#f59e0b] transition-colors"
              placeholder="e.g., 1000 units/month" />
          </div>
          <div>
            <label className="text-text-secondary text-xs font-semibold mb-1.5 block">Average Lead Time</label>
