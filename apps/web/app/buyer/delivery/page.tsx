...
import { useBuyerDeliveries } from '@/hooks/use-smart-delivery';
import { Package, AlertCircle, Search, RefreshCw, Eye, Truck, CheckCircle, Clock } from 'lucide-react';
import Link from 'next/link';

const statusTabs = ['ALL', 'OUT_FOR_DELIVERY', 'DELIVERED', 'DELIVERY_CONFIRMED', 'DELIVERY_FAILED', 'PARTIALLY_DELIVERED', 'REJECTED', 'COMPLETED'];

export default function BuyerDeliveryDashboard() {
  const [status, setStatus] = useState('ALL');
  const [search, setSearch] = useState('');
  const { data, isLoading, error } = useBuyerDeliveries(status === 'ALL' ? undefined : status);
  ...
  return (
    <div className="space-y-6">
      <DashboardPageHeader title="My Deliveries" description="Track and confirm your deliveries" />
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard icon={Truck} label="Completed" value={String(completed.length)} changeType="positive" />
      </div>
        ...
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.04] backdrop-blur-xl">
          ...
          {deliveries.map((d: any) => (
            <div key={d.id} className="grid grid-cols-1 gap-3 border-b border-white/[0.06] px-6 py-4 last:border-0 sm:grid-cols-12 sm:items-center">
              ...
              <p className="text-sm text-white/70 sm:col-span-2">{d.proofOfDelivery?.receiverName ?? d.receiverName ?? '—'}</p>
              <p className="text-xs text-white/50 sm:col-span-2">{new Date(d.createdAt).toLocaleDateString('en-IN')}</p>
              <div className="sm:col-span-2 text-right">
                <Link href={`/buyer/delivery/${d.id}`}><Button variant="ghost" size="sm"><Eye className="h-4 w-4" /></Button></Link>
              </div>
            </div>
          ))}
        </div>
