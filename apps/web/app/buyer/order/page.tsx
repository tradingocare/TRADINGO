...
import { useBuyerOrders } from '@/hooks/use-smart-order';
import { ShoppingCart, AlertCircle, Search, RefreshCw, Eye, Package } from 'lucide-react';
...
export default function BuyerOrderDashboard() {
  ...
  const { data, isLoading, error } = useBuyerOrders(status === 'ALL' ? undefined : status);
  const orders = data?.data ?? [];

  const activeOrders = orders.filter((o: any) => !['COMPLETED', 'CANCELLED', 'RETURNED', 'DELIVERED'].includes(o.status));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard icon={ShoppingCart} label="Active" value={String(activeOrders.length)} changeType="positive" />
        <StatCard icon={Eye} label="Delivered" value={String(orders.filter((o: any) => o.status === 'DELIVERED').length)} changeType="positive" />
        <StatCard icon={AlertCircle} label="Cancelled" value={String(orders.filter((o: any) => o.status === 'CANCELLED').length)} />
      </div>
      ...
      {error ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-white/[0.06] bg-white/[0.04] p-12 backdrop-blur-xl">
          ...
          <p className="mt-1 text-sm text-white/60">Orders will appear here once generated from a locked purchase order.</p>
        </div>
      ) : (
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.04] backdrop-blur-xl">
          ...
          {orders.map((order: any) => (
            ...
            >
              ...
              <p className="text-sm text-white/70 sm:col-span-3">{order.sellerCompany?.name ?? '—'}</p>
