...
import { useBuyerShipments } from '@/hooks/use-smart-shipment';
import { Package, AlertCircle, Search, RefreshCw, Eye, Truck, Clock, CheckCircle } from 'lucide-react';
...
export default function BuyerShipmentDashboard() {
  const [status, setStatus] = useState('ALL');
  const [search, setSearch] = useState('');
  const { data, isLoading, error } = useBuyerShipments(status === 'ALL' ? undefined : status);
  ...
  return (
    <div className="space-y-6">
      <DashboardPageHeader
        title="My Shipments"
        description="Track your incoming shipments"
      />

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard icon={Package} label="Total Shipments" value={String(shipments.length)} />
        <StatCard icon={Clock} label="In Transit" value={String(inTransit.length)} />
        <StatCard icon={CheckCircle} label="Delivered" value={String(delivered.length)} changeType="positive" />
        <StatCard icon={AlertCircle} label="Issues" value={String(shipments.filter((s: any) => s.status === 'DELIVERY_FAILED').length)} />
      </div>
        ...
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.04] backdrop-blur-xl">
          ...
          {shipments.map((s: any) => (
            <div key={s.id} className="grid grid-cols-1 gap-3 border-b border-white/[0.06] px-6 py-4 last:border-0 sm:grid-cols-12 sm:items-center">
              ...
              <p className="text-xs font-mono text-white/50 sm:col-span-1">{s.trackingNumber ?? '—'}</p>
