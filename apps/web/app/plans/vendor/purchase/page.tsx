...
import {
  CheckCircle2, Sparkles, ArrowRight, CreditCard, Building2, User, Mail,
...
function PurchasePageContent() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      toast.error('Failed to load plans or profile');
      console.error('Vendor purchase load error:', err);
  ...
  return (
    <div className="min-h-screen bg-bg-base">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-text-primary font-black text-2xl sm:text-3xl">Complete Your Purchase</h1>
          <p className="text-text-tertiary text-sm mt-1">Review and confirm your membership plan details</p>
        </div>
          ...
          <div className="lg:col-span-3 space-y-5">
            <div className="rounded-xl p-5 border border-border bg-surface">
              <h2 className="text-text-primary font-semibold text-sm mb-4 flex items-center gap-2">
                <Building2 size={14} className="text-[#f59e0b]" /> Company Information
              </h2>
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-surface">
                    <f.icon size={12} className="text-text-tertiary" />
                    <div>
                      <p className="text-text-tertiary text-[9px]">{f.label}</p>
                      <p className="text-text-secondary text-xs font-medium">{f.value || '—'}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-xl p-5 border border-border bg-surface">
              <h2 className="text-text-primary font-semibold text-sm mb-4 flex items-center gap-2">
                <CreditCard size={14} className="text-[#f59e0b]" /> Order Details
              </h2>
              <div className="space-y-4">
                <div>
                  <label className="text-text-secondary text-xs font-semibold mb-1.5 block">Plan Duration</label>
                  <div className="flex gap-2">
                    {[
                      ...
                      <button key={d.id} onClick={() => setDuration(d.id)}
                        ...
                        style={{
                          background: duration === d.id ? 'rgba(245, 158, 11, 0.12)' : 'var(--bg-elevated)',
                          border: duration === d.id ? '1px solid rgba(245, 158, 11, 0.35)' : '1px solid var(--border-color)',
                          color: duration === d.id ? '#f59e0b' : 'rgba(255,255,255,0.5)',
                        }}>{d.label}</button>
                    ))}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input value={coupon} onChange={e => setCoupon(e.target.value)}
                      className="w-full px-4 py-2.5 rounded-xl text-text-primary text-sm bg-surface border border-border focus:outline-none focus:border-[#f59e0b]"
                      placeholder="Optional" />
                  </div>
                  <div>
                    <label className="text-text-secondary text-xs font-semibold mb-1.5 block">Referral Code</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-text-secondary text-xs font-semibold mb-1.5 block">RM Code</label>
                    <input value={rmCode} onChange={e => setRmCode(e.target.value.toUpperCase())}
                      className="w-full px-4 py-2.5 rounded-xl text-text-primary text-sm bg-surface border border-border focus:outline-none focus:border-[#f59e0b]"
                      placeholder="Optional: RM + 5 digits" maxLength={7} />
                  </div>
                  <div className="flex items-end">
                    <Checkbox checked={gstInvoice} onChange={e => setGstInvoice(e.target.checked)} label="GST Invoice Required" className="pb-2.5" />
                  </div>
                </div>

                <div>
                  <label className="text-text-secondary text-xs font-semibold mb-1.5 block">Billing Address</label>
                  <textarea value={billingAddress} onChange={e => setBillingAddress(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl text-text-primary text-sm bg-surface border border-border focus:outline-none focus:border-[#f59e0b] min-h-[60px]"
            ...
            <div className="rounded-xl p-5 border border-border bg-surface">
              <h2 className="text-text-primary font-semibold text-sm mb-4 flex items-center gap-2">
                <Shield size={14} className="text-[#f59e0b]" /> Payment Method
              </h2>
              <div className="grid grid-cols-3 gap-2">
                {PROVIDERS.map(p => (
                  <button key={p.key} onClick={() => setGateway(p.key)}
                    ...
                    style={{
                      background: gateway === p.key ? 'rgba(245, 158, 11, 0.12)' : 'var(--bg-elevated)',
                      border: gateway === p.key ? '1px solid rgba(245, 158, 11, 0.35)' : '1px solid var(--border-color)',
                      color: gateway === p.key ? '#f59e0b' : 'rgba(255,255,255,0.5)',
                    }}>
                    <span>{p.icon}</span>
                    <span>{p.name}</span>
                  </button>
            ...
            <Checkbox checked={agreed} onChange={e => setAgreed(e.target.checked)}
              label="I agree to the Terms & Conditions and authorize TRADINGO to charge the selected amount." />
          </div>

          <div className="lg:col-span-2">
            <div className="rounded-xl p-5 border border-border bg-surface sticky top-8">
              <h2 className="text-text-primary font-bold text-sm mb-4">Order Summary</h2>
              {plan && (
                <div className="space-y-3">
                  <div className="flex justify-between">
                    <span className="text-text-tertiary text-xs">{plan.name} ({tier})</span>
                    <span className="text-text-primary font-semibold text-sm">₹{(total * duration).toLocaleString('en-IN')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-text-tertiary text-xs">Duration</span>
                    <span className="text-text-secondary text-xs">{duration} Year{duration > 1 ? 's' : ''}</span>
                  </div>
                  <div className="h-px bg-surface-secondary" />
                  <div className="flex justify-between">
                    <span className="text-text-primary text-sm font-bold">Total</span>
                    <span className="text-text-primary font-black text-lg">₹{(total * duration).toLocaleString('en-IN')}</span>
                  </div>
                  ...
                  {step === 'form' ? (
                    ...
                    <motion.button whileHover={{ scale:1.02 }} whileTap={{ scale:0.97 }}
                      ...
                      style={{ background:'linear-gradient(135deg,#f59e0b,#fbbf24)', color:'#fff' }}>
                      {processing ? <><LoadingSpinner size="xs" /> Processing...</> : <>Pay ₹{(total * duration).toLocaleString('en-IN')}</>}
                    </motion.button>
                  )}
                </div>
              )}
...
export default function PurchasePage() {
  return (
    <Suspense
      fallback={
        <div
          className="min-h-screen flex items-center justify-center bg-bg-base"
        >
          <div className="w-12 h-12 rounded-full border-2 border-t-[#f59e0b] border-border animate-spin" />
        </div>
      }
