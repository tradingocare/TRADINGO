...
import { CheckCircle2, X, Zap, Crown, Star, Shield, Sparkles, ArrowRight, Info } from 'lucide-react'
...
export default function PlansPageClient() {
  ...
  const getPrice = (plan: Plan, t: string) => {
    return t === 'B' ? plan.pricePlanB : t === 'C' ? plan.pricePlanC : plan.pricePlanA
  }
  ...
  const handleChoose = (planId: string) => {
    if (user) {
      ...
      const params = new URLSearchParams({ next: `/plans/vendor/purchase?planId=${planId}&tier=${tier}` })
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-12">
            ...
            return (
              <motion.div key={plan.id} initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ delay: i * 0.05 }}
                ...
                <motion.button whileHover={{ scale:1.02 }} whileTap={{ scale:0.97 }}
                  ...
                  {user ? 'Choose Plan' : 'Get Started'} <ArrowRight size={14} />
                </motion.button>
              </motion.div>
            )
          })}
        ...
        <div className="text-center mt-8">
          <p className="text-white/30 text-xs">
            All plans include GST invoice. Prices are in INR. Plans auto-renew unless cancelled.
          </p>
        </div>
      </div>
    </div>
