...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  try {
    ...
    return {
      ...
      alternates: {
        canonical: `https://tradingo.in/trading/${slug}`,
      },
      openGraph: {
        title: product.name,
        description: product.shortDescription || product.description?.slice(0, 160),
...
export default async function TradingProductPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  return (
    <Suspense fallback={<ProductSkeleton />}>
      <TradingProductDetail slug={slug} />
    </Suspense>
  );
}

async function TradingProductDetail({ slug }: { slug: string }) {
